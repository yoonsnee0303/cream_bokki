...
class WorkerThread_list_get(QThread):
    ...
    def run(self):
        ...
        # def sql_proc(conn, sql_statements):
        ...
        def get_list(date):
            ...
            for brand in brand_lists:
                if brand == '11':
                    ...
                    for pg in range(1,9): #41
                        ...
                        for i in range(len(response_json['commonPrdList']['items'])):
                            ...
                            #####
                            sql_statements = [
                                f"insert into table1 (url) values ('{u}');"]
                    ...
                    for i in range(len(temp)):
                        ...
                        #####
                        # save sql db
                        sql_statements = [f"insert into table1 (url) values ('{url}');"]
                        sql_proc(sql_statements)

                        # lotte_lists.append(url)
                        # all_lists.append(url)
                        # print(f'롯데온 {len(lotte_lists)}')
                        print(f'롯데온 {len(all_lists)}')

                        
                        # log_update2
                        # self.log_update2.emit(f"상세페이지 총 {len(lotte_lists)}개 수집")
                        self.log_update2.emit(f"상세페이지 총 {len(all_lists)}개 수집")
                    ...
                    for page in range(1, 23):
                        ...
                        for i in range(cnt):
                            ...
                            #####
                            sql_statements = [f"insert into table1 (url) values ('{ea_url}');"]
                            sql_proc(sql_statements)
                            # nav_lists.append(ea_url)
                            # all_lists.append(ea_url)
                            self.log_update2.emit(f"상세페이지 총 {len(all_lists)}개 수집")
                            print(f'스마트스토어 {len(all_lists)}')
                            print(ea_url)
                    ...
                    while True:
                        ...
                        for el in range(len(elems)):
                            ...
                            #####
                            sql_statements = [f"insert into table1 (url) values ('{temp}');"]
                            sql_proc(sql_statements)
                            # o_lists.append(temp)
                    ...
                    while True:
                        ...
                        if ck_end == 1: # 더 이상 상품이 나오지 않을 때 break
                            ...
                            sin_list = list(set(sin_list)) # 중복제거
                            # with open('sin_list.csv', 'w', newline='', encoding='utf-8-sig') as f:
                            #     write = csv.writer(f)
                            #     write.writerows([sin_list])
                            print('수집종료')
                            self.log_update2.emit(f"{date} 상세페이지 파일 업로드")
                            break
                            ...
                            for el in range(len(elems)):
                                ...
                                self.progress_update.emit(test)
                                url = elems[el]['id'].replace('item_unit_','https://www.ssg.com/item/itemView.ssg?itemId=')
                                print(url)
                                #####
                                sql_statements = [f"insert into table1 (url) values ('{url}');"]
                                sql_proc(sql_statements)
                                # all_list.append(url)
                                # all_lists.append(url)
                                # self.log_update2.emit(f"상세페이지 총 {len(all_list)}개 수집")   
                                self.log_update2.emit(f"상세페이지 총 {len(all_lists)}개 수집")   

                                print(url)

                            # print(f'신세계 {len(all_list)}')
                            print(f'신세계 {len(all_lists)}')
                    ...
                    for tag in range(len(a_tags)):
                        ...
                        #####
                        sql_statements = [f"insert into table1 (url) values ('{gm}');"]
                        sql_proc(sql_statements)
                        # gm_lists.append(gm)
                        # all_lists.append(gm)
                    ...
                    for pg in range(1,6):
                        ...
                        for href in hrefs:
                            #####
                            sql_statements = [f"insert into table1 (url) values ('{href}');"]
                            sql_proc(sql_statements)
                            
                            # auc_lists.append(href)
                            # all_lists.append(href)
                            # self.log_update2.emit(f"상품페이지 총 {len(auc_lists)}개 수집")
                            self.log_update2.emit(f"상품페이지 총 {len(all_lists)}개 수집")

                            # print(f'옥션 {len(auc_lists)}')
                            print(f'옥션 {len(all_lists)}')
                        ...
                        try:
                            ...
                            for j in range(cnt+1):
                                try:
                                    ...
                                    #####
                                    sql_statements = [f"insert into table1 (url) values ('{item_url}');"]
                                    sql_proc(sql_statements)
                                    # url_data.append(item_url)
                                    # all_lists.append(item_url)
                                    self.log_update2.emit(f"상세페이지 총 {len(all_lists)}개 수집")
                                    print(f'인터파크 {len(all_lists)}')
                                    print(item_url)
                                except:
                                    pass
                        except:
                    ...
                    # 더보기버튼
                    see_more_button = driver.find_element(By.CLASS_NAME,'scp-component-filter-options__fold-items')
                    see_more_button.click()

                    ul_elements = driver.find_elements(By.CLASS_NAME,'scp-component-filter-options__option-items__btn-fold') # 열림 버튼
                    ...
                    for ul_element in ul_elements:
                        driver.execute_script("arguments[0].click();", ul_element) # 열림 버튼 클릭
                    ...
                    def process_urls(tag_list):
                        for i in range(1,165):
                            # log
                            test = int((ratio/len(tag_list)*tag_list.index(tag)+ratio*8))
                            self.progress_update.emit(test)
                            # tag = tag_list[i]
                ...
                cnt += 1
                detail_url = process_urls(tag_list)
            ...
            # sql_2 = ['select * from table1 Limit10;']
            # sql_proc(conn,sql_2)
            # print('ck1')

            # sql_3 = ['select * from table1 ORDER BY id DESC Limit20;']
            # sql_proc(conn,sql_2)
            # print('ck2')

            # for url in all_lists:
            # with open('all_lists.csv', 'w', newline='', encoding='utf-8-sig') as f:
            #     write = csv.writer(f)
            #     write.writerows([all_lists])
            print('done')
